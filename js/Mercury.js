...
function createNewAccount() {
  ...
  M.toast({html: 'Building the new account!', classes: 'rounded blue accent-4'});
...
function deleteAccount(obj) {
  ...
  if (ipcr === 0) {
    M.toast({html: 'Account deleted!', classes: 'rounded red accent-4'});
...
//    M.toast({html: 'Account updated!', classes: 'rounded blue accent-4'});
}
...
function createNewOperation(data) {
  ...
  let oper = '<i class="material-icons left white-text">monetization_on</i>Operation created for: <b class="white-text"> ' + account + '</b>'
  M.toast({html: oper, classes: 'rounded white-text accent-4'});
...
function deleteOp(id) {
  ...
  if (confirm === 0) {
    ...
    updateSQL('#account');
    showUnsavedTag();
    M.toast({html: 'Operation deleted!', classes: 'rounded red accent-4'});
  }
  resetOp(globSettings);
  updateAccountsList();
  try {
