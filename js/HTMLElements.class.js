const HTMLElements = {
    ...
    acBar: function (accountList) {
        return $('<nav>')
            ...
            .addClass('field is-grouped nav-wrapper N/A transparent')
            .attr('id', 'acBar')
    ...
    stateFilter: function () {
        const options = {
            options: [
        ['*', i18njs('All')],
        ['fa fa-circle-o', i18njs('Register')],
    ...
    acTable: function () {
        return $('<table>').addClass('table')
            .append(
                $('<thead>').append(
                    $('<tr>')
                    .append($('<th>').text(i18njs('State')).addClass('has-text-centered black'))
                    .append($('<th>').text(i18njs('Date')).addClass('has-text-centered black'))
                    .append($('<th>').text(i18njs('Type')).addClass('has-text-centered black'))
                    .append($('<th>').text(i18njs('Beneficiary', 1)).addClass('black'))
                    .append($('<th>').text(i18njs('Category', 1)).addClass('black'))
                    .append($('<th>').text(i18njs('Label')).addClass('black'))
                    .append($('<th>').text(i18njs('Debit')).addClass('has-text-centered black'))
                    .append($('<th>').text(i18njs('Collection')).addClass('has-text-centered black'))
                )
            )
            .append(
        ...
        return $('<div>').addClass('tile ancestor notification is-dark is-paddingless')
            .append(
                ...
                .append(
                    ...
                    .append(
                        ...
                        .append($('<p>').addClass('level-left title').text(i18njs('Previous Expenses')))
                        .append(
            ...
            .append(
                ...
                .append(
                    ...
                    .append(
                        $('<div>').addClass('level is-marginless')
                        .append($('<p>').addClass('level-left title is-marginless').text(i18njs('Current Expenses')))
                        .append(
                            $('<p>').addClass('level-right subtitle is-6 is-marginless')
                            .text(moment().subtract(0, 'months').format('MMMM YYYY'))
                            .attr('style', 'position: relative; top: 0.35em')
                        )
                    )
                    .append(
    ...
    addCustomAction: function (i18n, text, color, icon, ipcmsg) {
        return $('<li>').addClass('subtitle is-5')
            .append($('<span>').text("• " + i18njs("Open ", i18n) + " "))
...
//    \\\\\\\\\\\ NEW OUTCOME ////////////////
        topChartNew: function () {
        const ctx = $("#doughnut1");
        const firstDateofMonth = moment().subtract(0, 'months').startOf('month').format('YYYY-MM-DD');
        const lastDateofMonth = moment().subtract(0, 'months').endOf('month').format('YYYY-MM-DD');
        let data_db;
        try {
            data_db = global.db.exec(`SELECT category, SUM(amount) as s FROM OPERATION WHERE amount<=0 AND date BETWEEN "${firstDateofMonth}" AND "${lastDateofMonth}" GROUP BY category ORDER BY s ASC LIMIT 6`)
        } catch (e) {
        ...
        let doughnut1 = new Chart(ctx, config);
    },
    ...
    recurrings: function () {
        return $('<div>')
            .append(
                ...
                .append(
                    $('<span>').addClass(' is-medium has-text-success')
                    //        .append($('<i>').addClass('fa fa-recycle'))
                    .append($('<i>').addClass('small material-icons left green-text').append($('<span>').text(i18njs('content_copy'))))
                )
    ...
    createRec: function () {
        return $('<div>').addClass('level-item')
            .append(
                $('<a>').addClass('blue-grey darken-4 waves-effect waves-light btn-large is-info')
                .attr('data-toggle', '#rec-modal')
                .attr('data-type', 'create')
                .attr('onclick', "showModal(this)")
                .append($('<i>').addClass('material-icons left').append($('<span>').text(i18njs('add'))))
    ...
    lauchRec: function () {
        return $('<div>').addClass('level-item')
            .append(
                $('<a>').addClass('blue-grey darken-4 waves-effect waves-light btn-large')
                .attr('data-toggle', '#rec-modal')
                .attr('data-type', 'launch')
                .attr('onclick', "showModal(this)")
                .append($('<i>').addClass('material-icons left').append($('<span>').text(i18njs('cloud'))))
    ...
    editRec: function () {
        return $('<div>').addClass('level-item')
            .append(
                $('<a>').addClass('blue-grey darken-4 waves-effect waves-light btn-large is-success')
                .attr('data-toggle', '#rec-modal')
                .attr('data-type', 'edit')
                .attr('onclick', "showModal(this)")
                .append($('<i>').addClass('material-icons left').append($('<span>').text(i18njs('create'))))
    ...
    datePicker: function () {
        return new CustomField('calendar', 'rec-date', {
            placeholder: i18njs('Pick a date'),
            style: i18njs('margin:0; flex:1;')
        }, null, 'text').generate();
    },

    amountPicker: function () {
        return new CustomField(globSettings.defaultCurrency, 'rec-amount', {
            placeholder: '0.00'
        }, null, 'number').generate();
    },
    ...
    beneficiaryPicker: function () {
        return new CustomField('building-o', 'rec-benef', {
            placeholder: i18njs('Beneficiary', 1)
        }, null, 'text').generate();
    },

    categoryPicker: function () {
        return new CustomField('flag', 'rec-cat', {
            placeholder: i18njs('Category', 1)
        }, null, 'text').generate();
    },

    labelPicker: function () {
        return new CustomField('tag', 'rec-label', {
            placeholder: i18njs('Label')
        }, null, 'text').generate();
    },
    ...
    editModal: function (accountList) {
        ...
        if ($('#recTable > .is-selected').get(0) === undefined) {
            $('#modalbody').append(
                ...
                $('<p>').addClass('title is-5').text(i18njs('No operation selected!'))
            )
        } else {
            $('#modalbody').append(
                $('<p>').addClass('title').text(i18njs('Edit operation'))
