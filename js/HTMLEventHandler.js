...
$('#btn-state').on('click', () => {
  ...
  if (icon.hasClass('fa-circle-o')) {
    ...
    let rev0 = (i18njs("Checked"))
    M.toast({html: rev0, classes: 'rounded light-blue-text lighten-5'})
  } else if (icon.hasClass('fa-circle')) {
    icon.removeClass();
    icon.addClass('fa fa-check-circle')
    let ver1= (i18njs("Verified"))
    M.toast({html: ver1, classes: 'rounded bold green-text accent-4'})
  } else if (icon.hasClass('fa-check-circle')) {
    icon.removeClass();
    icon.addClass('fa fa-circle-o')
    let ver2 = (i18njs("Register"))
    M.toast({html: ver2, classes: 'rounded grey-text lighten-5'})
  }
})
...
function tabToggle(event) {
  ...
  $('#dash-body').append(visible);
  if(sqlType==="#dashboard") HTMLElements.topChartNew();
  updateSQL(sqlType);
